...
**重装一遍就是更新。** 上面每条命令都会先删掉旧目录再复制,所以照用户所用 agent 的那一节
重跑一次,就把 skill 换成了最新版。

有两样东西在更新时不会受影响,这是设计好的:

- **配置**(`SIYUAN_URL`、`SIYUAN_TOKEN`)在 skill 目录之外,凭据永远不需要重填。
  更新时跳过第 3、4 步,确认配置文件还在即可。
- **用户笔记库里的任何内容都不会被碰到。**

如果用户的 agent 只在会话启动时加载 skill,更新完要提醒他们重启。
